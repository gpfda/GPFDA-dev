...
repgp.loglikelihood=function(hyper.p,response,Data,Cov,gamma=1,nu=1.5,time=NULL,...){
...
repgp.Dloglikelihood=function(hyper.p, response,Data,Cov,gamma=1,nu=1.5,time=NULL,...){#,Xprior=prior_D1_likelihood,Xprior2=prior_likelihood){
  ### response is expected to be matrices with ncol replications and nrow observations
  ### Data is expected to be a list with matrices
...
fisherinfo=function(pp.cg,X,Y,Cov,gamma,nu){
  ...
  D2fx=lapply(seq_along(pp.cg),function(i){
    ...
    if(name.Dp%in%c('linear.a'))
      D2para=f(hyper=pp.cg, data=X, Alpha.Q=AlphaQ)
    if(name.Dp%in%c('linear.i'))
      D2para=f(hyper=pp.cg, inv.Q=invQ, Alpha.Q=AlphaQ)
    if(!name.Dp%in%c('pow.ex.w','pow.ex.v','matern.w','matern.v','linear.a','linear.i'))
      D2para=f(hyper=pp.cg, data=X, inv.Q=invQ, Alpha.Q=AlphaQ)
    return(D2para)
  })
...
#   hhhhp<<-init0;dadada<<-Data;rpp<<respo
  ...
  pp=nlminb(init0,repgp.loglikelihood,repgp.Dloglikelihood,response=response,Data=Data,Cov=Cov,gamma=gamma,nu=nu,
  ...
  Qlist=lapply(allbat,function(l) fisherinfo(pp.cg=pp.cg,X=as.matrix(l[,-1]),Y=l[,1],Cov=Cov,gamma=gamma,nu=nu))
  ...
  result=list('hyper'=pp.cg,'I'=II, 'modellist'=model$model,'CovFun'=Cov,'gamma'=gamma,'nu'=nu,'init_resp'=y_raw,meanFM=mean,
...
gpfr=function(response,lReg=NULL,fReg=NULL,fyList=NULL,fbetaList_l=NULL,fxList=NULL,
  ...
  a1<-gpfrtrain(response=response,lReg=lReg,fReg=fReg,gpReg=gpReg,fyList=fyList,
                fbetaList_l=fbetaList_l,fxList=fxList,fbetaList_f=fbetaList_f,
                fbetaList=fbetaList,hyper=hyper,Cov=Cov,gamma=gamma,nu=nu,useGradient=useGradient,
                fitting=fitting,
                time=time,rPreIdx=rPreIdx,accuracy=accuracy,trace.iter=trace.iter,
