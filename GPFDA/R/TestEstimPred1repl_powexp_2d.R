...
Cov <- c('pow.ex')
hp <- list('pow.ex.v'=log(2),'pow.ex.w'=c(log(20), log(50)),'vv'=log(0.3))
gam <- 2

nSamples <- 1
X0 <- X <- as.matrix(expand.grid(x1=x1, x2=x2))

Sigma <- cov.pow.ex(hyper = hp, Data = X0, gamma = gam)
...
SigmaHat <- cov.pow.ex(hyper = aNew$hyper, Data = X0, gamma=gam)
