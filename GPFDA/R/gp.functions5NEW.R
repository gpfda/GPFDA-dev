...
cov.linear=function(hyper,Data,Data.new=NULL){
  ...
  cov.lin=xixjNEW(X=Data.new, Xnew=Data, A = A)
  return(cov.lin)
}
...
cov.pow.ex=function(hyper,Data,Data.new=NULL,gamma=1){
  ...
  v.power=xixj_staNEW(X = Data, Xnew = Data.new,A=A,power=gamma)
...
cov.rat.qu=function(hyper,Data,Data.new=NULL){
  ...
  v.power=xixj_staNEW(Data,Data.new,A=A,power=1)
...
######################## Predict ############################
gppredictNEW=function(train=NULL,Data.new=NULL,noiseFreePred=F,hyper=NULL, Data=NULL, Y=NULL, Cov=NULL,gamma=NULL,lrm=NULL,mean=0){
...
gprNEW=function(Data, response, Cov=c('linear','pow.ex'), m = NULL, hyper=NULL, NewHyper=NULL, mean=0, gamma=1,itermax=100,reltol=8e-10,trace=0){#,Xprior,Xprior2){
#   set.seed(60);
  Data=as.matrix(Data)
  ...
  CG0 <- nlminb(unlist(hyper), gp.loglikelihood2NEW, gp.Dlikelihood2NEW,
                Data=Data,response=response,Cov=Cov,gamma=gamma,
  ...
  result=list('hyper'=hyper.cg,'I'=II,'fitted.mean'=fitted[,1],fitted.sd=sqrt(fitted.var),
              ...
              'CovFun'=Cov,'gamma'=gamma,'Q'=Q,'inv'=invQ,'mean'=mean,
...
Dpow.ex=function(vec,Data,hyper,Q=NULL,gamma){
  DQ=cov.pow.ex(hyper,Data,gamma=gamma)
  vec=as.matrix(vec)
  A=as.matrix(exp(hyper$pow.ex.w[which(apply(Data,2,mean)==mean(vec) & apply(Data,2,max)==max(vec) & apply(Data,2,min)==  min(vec))]))
  DQ=-0.5*DQ*xixj_staNEW_sq(vec,A=A,power=gamma)
  return(DQ)
}
...
#   DQ=DQ%*%xixj_staNEW(as.matrix(vec),
#                       w=as.matrix(exp(hyper$rat.qu.w[which(apply(Data,2,mean)==mean(vec) & apply(Data,2,max)==max(vec) & apply(Data,2,min)==  min(vec))])),
#                       power=1)
#   return(DQ)
# }
...
Drat.qu=function(vec,Data,hyper,Q=NULL){
  ...
  power=exp(hyper$rat.qu.a)
  DQ=-power*hyper$rat.qu.s*DQ^((power+1)/power)
  A=as.matrix(exp(hyper$rat.qu.w[which(apply(Data,2,mean)==mean(vec) & apply(Data,2,max)==max(vec) & apply(Data,2,min)==  min(vec))]))
  DQ=DQ%*%xixj_staNEW_sq(vec,A=A,power=1)
  return(DQ)
}

# D2pow.ex=function(vec,Data,hyper,Q=NULL,gamma){
#   DQ=cov.pow.ex(hyper,Data,gamma=gamma)
#   extra=xixj_staNEW(as.matrix(vec),
#                     w=as.matrix(exp(hyper$pow.ex.w[which(apply(Data,2,mean)==mean(vec) & apply(Data,2,max)==max(vec) & apply(Data,2,min)==  min(vec))])),
#                     power=gamma)
...
D2pow.ex=function(vec,Data,hyper,Q=NULL,gamma){
  ...
  vec=as.matrix(vec)
  A=as.matrix(exp(hyper$pow.ex.w[which(apply(Data,2,mean)==mean(vec) & apply(Data,2,max)==max(vec) & apply(Data,2,min)==  min(vec))]))
  extra=xixj_staNEW_sq(vec,A=A,power=gamma)
  D2Q=0.25*DQ*extra^2-0.5*DQ*extra
  return(D2Q)
}
...
#   power=exp(hyper$rat.qu.a)
#   extra=hyper$rat.qu.s*xixj_staNEW(as.matrix(vec),
#                                    w=as.matrix(exp(hyper$rat.qu.w[which(apply(Data,2,mean)==mean(vec) & apply(Data,2,max)==max(vec) & apply(Data,2,min)==  min(vec))])),
#                                    power=1)
...
D2rat.qu=function(vec,Data,hyper,Q=NULL){
  ...
  power=exp(hyper$rat.qu.a)
  A=as.matrix(exp(hyper$rat.qu.w[which(apply(Data,2,mean)==mean(vec) & apply(Data,2,max)==max(vec) & apply(Data,2,min)==  min(vec))]))
  extra=hyper$rat.qu.s*xixj_staNEW_sq(vec,A=A,power=1)
  DQ=(-power)*((-power-1)*(extra^2)*Q^((power+2)/power)+extra*Q^((power+1)/power))
  return(DQ)
}

# DCov.linear.a=function(hyper,data,AlphaQ){
#   Dlinear.aj=apply(data,2,function(i) sum(AlphaQ*exp(hyper$linear.a[which(data[1,]==i[1])])*xixjNEW_sq(X=as.matrix(i),A=as.matrix(1))) )
#   return(Dlinear.aj)
# }

DCov.linear.a=function(hyper,data,AlphaQ){
  Dlinear.aj=apply(data,2,function(i) {
    mat_i <- as.matrix(i)
    mat_1 <- as.matrix(1)
    sum(AlphaQ*exp(hyper$linear.a[which(data[1,]==i[1])])*xixjNEW_sq(X=mat_i,A=mat_1))})
  return(Dlinear.aj)
}
...
#   hyper=lapply(hyper,exp);
#   v.power=xixj_staNEW(data,data,as.matrix(hyper$rat.qu.w),power=1)
...
DCov.rat.qu.s=function(hyper,data,AlphaQ){
  hyper=lapply(hyper,exp)
  A=as.matrix(hyper$rat.qu.w)
  v.power=xixj_staNEW_sq(data,A=A,power=1)
  Drat.qu.s=(-hyper$rat.qu.a)*v.power*(1+log(hyper$rat.qu.s)*v.power)^(-hyper$rat.qu.a-1)
  Drat.qu.s=sum(AlphaQ*Drat.qu.s)
  return(Drat.qu.s)
}
...
#   D2linear.aj=apply(data,2,function(i) D2(exp(hyper$linear.a[which(data[1,]==i[1])])*xixjNEW_sq(X=as.matrix(i),A=as.matrix(1)),exp(hyper$linear.a[which(data[1,]==i[1])])*xixjNEW_sq(X=as.matrix(i),A=as.matrix(1)),inv.Q,Alpha.Q))
#   return(D2linear.aj)
# }
D2linear.a=function(hyper,data,inv.Q,Alpha.Q){
  D2linear.aj=apply(data,2,function(i) {
    mat_i <- as.matrix(i)
    mat_1 <- as.matrix(1)
    D2(exp(hyper$linear.a[which(data[1,]==i[1])])*xixjNEW_sq(X=mat_i,A=mat_1),
       exp(hyper$linear.a[which(data[1,]==i[1])])*xixjNEW_sq(X=mat_i,A=mat_1),
       inv.Q, Alpha.Q)})
  return(D2linear.aj)
}
...
gp.loglikelihood2NEW=function(hyper.p,Data, response,Cov,gamma=1){
...
#this function is for the conjugate gradian
#hyper is a list of hyper-parameters
#Data should have the form that, each column is a variable
#response is the given response vector
#Cov is a function contains all the covariance matrix, defult is:
###cov.linear(hyper,Data)+cov.pow.ex(hyper,Data)
gp.Dlikelihood2NEW=function(hyper.p,  Data, response,Cov,gamma){
