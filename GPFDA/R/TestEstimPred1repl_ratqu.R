...
nSamples <- 1
# hp <- list('rat.qu.a'=log(20), 'rat.qu.v'=log(7),'rat.qu.w'=log(10), 'vv'=log(1))
hp <- list('rat.qu.a'=log(5), 'rat.qu.v'=log(7),'rat.qu.w'=log(20), 'vv'=log(1))
h <- seq(0,1,len=n)
idx <- sort(sample(1:n,floor(n/2)))
X <- as.matrix(h[idx])

Sigma <- cov.rat.qu(hyper = hp, Data = h)
...
t0 <- proc.time()
aNew <- gprNEW(Data=X, response=Y, Cov=c('rat.qu'),trace=2, useGradient=T, itermax=50,
               nInitCandidates = 30)
...
SigmaHat <- cov.rat.qu(hyper = aNew$hyper, Data = h)
