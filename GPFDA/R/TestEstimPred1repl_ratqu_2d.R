...
Cov <- c('rat.qu')
hp <- list('rat.qu.a'=log(5), 'rat.qu.v'=log(7),'rat.qu.w'=c(log(20), log(50)), 'vv'=log(1))

nSamples <- 1
X0 <- X <- as.matrix(expand.grid(x1=x1, x2=x2))

Sigma <- cov.rat.qu(hyper = hp, Data = X0)
...
t0 <- proc.time()
aNew <- gprNEW(Data=X, response=Y, Cov=c('rat.qu'),trace=2, useGradient = T,
               itermax=50, nInitCandidates = 30)
newGPFDA_time <- proc.time() - t0

exp(unlist(hp)) # true
(newGPFDA_estimates <- round(exp(unlist(aNew$hyper)), 4))

SigmaHat <- cov.rat.qu(hyper = aNew$hyper, Data = X0)
